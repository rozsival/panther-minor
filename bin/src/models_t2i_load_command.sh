panther_t2i_load() {
  ...
  local diffusion uncond llm vae clip t5 model_dir
  ...
  clip="$(panther_t2i_component_target "$model" clip)"
  t5="$(panther_t2i_component_target "$model" t5)"
  ...
  panther_upsert_env_key "$PANTHER_ENV_FILE" SD_CPP_CLIP_L "$clip"
  panther_upsert_env_key "$PANTHER_ENV_FILE" SD_CPP_T5XXL "$t5"
  ...
  panther_log_info "Set Open WebUI's image model to '$model' (admin image settings) to generate with it in chat."
}
