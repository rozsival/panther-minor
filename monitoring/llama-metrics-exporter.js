...
function shouldLog(level) {
	return (
		(LOG_PRIORITY[level] ?? LOG_PRIORITY.info) <=
		(LOG_PRIORITY[LOG_LEVEL] ?? LOG_PRIORITY.info)
	);
}
...
export function normalizeModelsPayload(payload) {
	...
	for (const item of data) {
		...
		const statusValue =
			typeof item?.status?.value === 'string'
				? item.status.value.trim().toLowerCase()
				: 'unknown';
		seen.add(id);
...
export function exporterStatusLines(models, loadedModelId, metricsScrapeOk) {
	...
	for (const model of models) {
		...
		lines.push(
			`llama_metrics_exporter_model_loaded{model="${escapeLabelValue(model.id)}"} ${isLoaded ? 1 : 0}`,
		);
		lines.push(
...
export async function fetchMetricsText(model, fetchImpl = fetch) {
	...
	if (!response.ok) {
		...
		const error = new Error(
			`upstream metrics request failed for ${model}: ${response.status} ${response.statusText}`,
		);
		error.status = response.status;
		error.statusText = response.statusText;
		throw error;
...
export async function buildMetricsPayload(fetchImpl = fetch) {
	...
	if (loadedModel) {
		...
		try {
			modelToMetrics[loadedModel.id] = await fetchMetricsText(
				loadedModel.id,
				fetchImpl,
			);
