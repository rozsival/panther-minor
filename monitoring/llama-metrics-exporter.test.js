...
`.trim();
  ...
  const { aliasesByModelPath, knownAliases } =
    loadPresetAliasesFromText(preset);
  const payload = {
    slots: [
      {
        id: 0,
...
test("injectModelLabel adds label for unlabeled series", () => {
  ...
  assert.equal(
    ...
    'llamacpp:tokens_predicted_total{model="panther-minor"} 123',
  );
});
...
`;
  ...
  assert.equal(
    (merged.match(/# HELP llamacpp:tokens_predicted_total/g) ?? []).length,
    1,
  );
  assert.match(
    merged,
    /llamacpp:tokens_predicted_total\{model="panther-minor"} 100/,
  );
  assert.match(
    merged,
    /llamacpp:tokens_predicted_total\{model="panther-coder"} 200/,
  );
});
