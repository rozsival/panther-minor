...
test('injectModelLabel adds label for unlabeled series', () => {
	...
	assert.equal(
		...
		'llamacpp_tokens_predicted_total{model="panther-minor"} 123',
	);
});
...
`;
	...
	assert.equal(
		(merged.match(/# HELP llamacpp_tokens_predicted_total/g) ?? []).length,
		1,
	);
	assert.match(
		merged,
		/llamacpp_tokens_predicted_total\{model="panther-minor"} 100/,
	);
	assert.match(
		merged,
		/llamacpp_tokens_predicted_total\{model="panther-coder"} 200/,
	);
});
...
test('exporterStatusLines reports discovered and loaded states', () => {
	...
	assert.match(
		lines,
		/llama_metrics_exporter_model_loaded\{model="panther-minor"} 1/,
	);
	assert.match(
		lines,
		/llama_metrics_exporter_model_loaded\{model="panther-coder"} 0/,
	);
	assert.match(
		lines,
		/llama_metrics_exporter_model_up\{model="panther-minor"} 1/,
	);
	assert.match(
		lines,
		/llama_metrics_exporter_model_up\{model="panther-coder"} 0/,
	);
});
...
test('buildMetricsPayload scrapes metrics only for the single loaded model', async () => {
	...
	assert.doesNotMatch(
		payload,
		/llamacpp_tokens_predicted_total\{model="panther-coder"}/,
	);
});
...
test('buildMetricsPayload skips metrics scrape when no model is loaded', async () => {
	...
	const fetchImpl = async (url, _options) => {
		...
		throw new Error(
			'metrics endpoint should not be called without a loaded model',
		);
	};
	...
	assert.match(
		payload,
		/llama_metrics_exporter_model_up\{model="panther-minor"} 0/,
	);
	assert.match(
		payload,
		/llama_metrics_exporter_model_up\{model="panther-coder"} 0/,
