...
- `llama-manager/main.js` — activity-aware reverse proxy; records inference activity, exposes `/status` for the exporter; stops and restarts the `llama-cpp` container via Docker socket for scale-to-zero GPU power saving
